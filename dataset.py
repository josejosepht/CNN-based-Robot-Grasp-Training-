...
class GraspDataset(Dataset):
    ...
    def transform_grasp(self, img: Tensor, action: np.ndarray) -> Tuple[Tensor, np.ndarray]:
        ...
        '''
        #Random choice of angles
        randomAngle=np.random.choice([0.0,90.0,180.0,270.0])
        #rotating about a random angle
        image=TF.rotate(img, randomAngle)
        #unpack the action tuple
        px,py,rot_id=action
        #center the px,py of image before rotation
        px=px-(63/2)
        py=py-(63/2)
        #Define the angle of rotation
        rotMat=np.array([[np.cos(np.deg2rad(randomAngle)),-np.sin(np.deg2rad(randomAngle))]
                         ,[np.sin(np.deg2rad(randomAngle)),np.cos(np.deg2rad(randomAngle))]])
        #get new image plane points after rotation
        pNew=np.dot(rotMat,np.array([[px],[py]]))
        
        px=int(pNew[0]+31.5)
        py=int(pNew[1]+31.5)
        if (randomAngle==90.0 or randomAngle==270.0):
            rot_id=int(not(rot_id))
        action=(px,py,rot_id)
        return img, action
