...
class MobileUNet(nn.Module):
    def __init__(self, pretrained: bool=True, num_rots: int=2) -> None:
        ...
        Refer to the provided diagram for details on the layers : self.backbone1 
        is the first part of the MobileNet that produces a 24-channel, 8x8 
        feature map; self.backbone2 produces a 40-channel, 4x4 feature map.

        No batch norm layers are included in this implementation.  All conv1x1 
        layers have stride=1 and padding=0.  To perform upsampling of the 
        feature maps, we recommend using `nn.Upsample` and setting the argument
        `align_corners=True`.

        The two pathways in the network are combined using addition. The output
        of the network does not have an activation function applied to it
        '''
        super().__init__()
        ...
        ##########################
        # defining other layers here
        ##########################
        self.conv1=nn.Conv2d(in_channels=40, out_channels=128,kernel_size=1, stride=1,padding=0)
        self.relu1=nn.ReLU(True)
        self.upsample1=nn.Upsample(scale_factor=8,mode='bilinear',align_corners=True)
        self.conv2=nn.Conv2d(in_channels=128, out_channels=4,kernel_size=1,stride=1)
        self.conv3=nn.Conv2d(in_channels=4, out_channels=2,kernel_size=1, stride=1)
        self.relu2=nn.ReLU(True)
        self.upsample2=nn.Upsample(scale_factor=2,mode='bilinear',align_corners=True)
        
        self.upsample3=nn.Upsample(scale_factor=4,mode='bilinear',align_corners=True)
        self.conv4=nn.Conv2d(in_channels=24, out_channels=4,kernel_size=1, stride=1)
    ...
    def forward(self, img: Tensor) -> Tensor:
        ...
        ############################################
        # implementing additional steps in forward pass
        ############################################
        ...
        x_wide=self.upsample3(x_wide)
        x_wide=self.conv4(x_wide)
    ...
    @torch.no_grad()
    def predict(self, img: Tensor) -> np.ndarray:
